...
struct TestConfig {
    const char* param = "./models/PP-LCNet_x1_0_doc_ori.param";
    const char* bin = "./models/PP-LCNet_x1_0_doc_ori.bin";
...
int main(int argc, char** argv)
{
    ...
    if (!parse_args(argc, argv, cfg)) {
        std::cerr << "Usage: " << argv[0] << " [-p param] [-b bin] [-i image] [-g gpu] [--fp16] [--int8] [--bf16]" << std::endl;
        return 2;
    }

    std::cout << "LiteOCR Doc Orientation Classifier Test" << std::endl;
    ...
    liteocr_doc_ori_t classifier = liteocr_doc_ori_create();
    ...
    if (liteocr_doc_ori_load_model(classifier, cfg.param, cfg.bin, &opt) != 0) {
        std::cerr << "Failed to load doc orientation model." << std::endl;
        liteocr_free_image(&input);
        liteocr_doc_ori_destroy(classifier);
        return 1;
    }

    int orientation = liteocr_doc_ori_forward(classifier, &input);
    if (orientation < 0) {
        std::cerr << "Doc orientation forward failed." << std::endl;
        liteocr_free_image(&input);
        liteocr_doc_ori_destroy(classifier);
        return 1;
    }
    ...
    int orientation2 = liteocr_doc_ori_forward(classifier, &rot);
    if (orientation2 < 0) {
        std::cerr << "Doc orientation forward failed after rotation." << std::endl;
        free(rot.data);
        liteocr_free_image(&input);
        liteocr_doc_ori_destroy(classifier);
        return 1;
